...
-- 반응시간 구간별 평균 성공률
-- ========================================
-- 반응시간을 50ms 단위로 구간화하여
-- 각 구간별 평균 성공률 분석

SELECT
    CONCAT(
        FLOOR(s.avg_reaction_time / 50) * 50,
        ' ~ ',
        FLOOR(s.avg_reaction_time / 50) * 50 + 50,
        'ms'
    ...
    ROUND(AVG(s.avg_reaction_time), 1) AS avg_reaction_time_ms
FROM student s
...
GROUP BY FLOOR(s.avg_reaction_time / 50)
ORDER BY FLOOR(s.avg_reaction_time / 50) ASC;